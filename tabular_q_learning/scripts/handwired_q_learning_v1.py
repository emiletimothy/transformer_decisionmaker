...
Two context-write rules (QLearningConfig.residual):
  residual=False (overwrite, original): <Update> re-emits the whole new column,
      c_{a_t} <- h[Update]; Head 3.3 copies u_{a_t} and sum_s Q_t(s, a_t) u_s into
      <Update> and Layer 4 adds the TD increment on top.
  residual=True: c_{a_t} <- c_{a_t} + P_buf1 h[Update]; Head 3.3 is dropped, so buf1 of
      <Update> holds only the increment alpha (r + gamma max_a Q_t(s', a) - Q_t(s, a))
      u_{s_t}, and the old column and the slot's identity tag are carried unchanged.
"""
...
class QLearningConfig:
    ...
    gamma: float = 0.9
    beta: float = BETA
    residual: bool = False   # residual context write (drops Head 3.3), see module doc
...
class ContinuousThoughtQTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        if not cfg.residual:
            # Head 3.3 (overwrite only): <Update> inherits u_{a_t} and the column
            # sum_s Q_t(s, a_t) u_s from <a_t>, so that h[Update] is the whole new slot.
            l3_heads.append(SoftmaxHead(
        ...
        self.l3 = nn.ModuleList(l3_heads)
